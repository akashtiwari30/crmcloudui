                                    ...
                                    <div class="pull-right">
                                        ...
                                        {if $EXTENSION->isVtigerCompatible()}
                                            {if ($EXTENSION->isAlreadyExists()) and (!$EXTENSION_MODULE_MODEL->get('trial'))}
                                                {if ($EXTENSION->isUpgradable())}
                                                    {if $EXTENSION->get('isprotected') && $IS_PRO}
                                                        <button class="oneclickInstallFree btn btn-submit margin0px {if ($REGISTRATION_STATUS) and ($PASSWORD_STATUS)}authenticated {else} loginRequired{/if}">
                                            ...
                                            {elseif (($EXTENSION->get('price') eq 'Free') or ($EXTENSION->get('price') eq 0))}
                                                {if $EXTENSION->get('isprotected') && $IS_PRO}
                                                    <button class="oneclickInstallFree btn btn-submit {if ($REGISTRATION_STATUS) and ($PASSWORD_STATUS)}authenticated {else} loginRequired{/if}">{vtranslate('LBL_INSTALL', $QUALIFIED_MODULE)}</button>
                                                {elseif !$EXTENSION->get('isprotected')}
                                                    <button class="oneclickInstallFree btn btn-submit {if ($REGISTRATION_STATUS) and ($PASSWORD_STATUS)}authenticated {else} loginRequired{/if}">{vtranslate('LBL_INSTALL', $QUALIFIED_MODULE)}</button>
                                                {/if}
                                            {elseif ($IS_PRO)}
                                                {if ($EXTENSION->get('trialdays') gt 0) and ($EXTENSION_MODULE_MODEL eq 'false') and ($EXTENSION->get('isprotected') eq 1)}
                                                    <button class="oneclickInstallPaid btn btn-submit {if ($REGISTRATION_STATUS) and ($PASSWORD_STATUS)}authenticated {else} loginRequired{/if}" data-trial=true>{vtranslate('LBL_TRY_IT', $QUALIFIED_MODULE)}</button>
