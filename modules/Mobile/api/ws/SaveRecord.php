			...
			if (empty($recordid) || vtws_recordExists($recordid)) {
				...
				if ($module == 'Leads') {
					...
					if (!empty($mobile)) {
						global $adb;
						$result = $adb->pquery(
							"SELECT crmid FROM vtiger_potential 
							...
							WHERE vtiger_potentialscf.cf_861 = ? AND vtiger_crmentity.deleted = 0 AND vtiger_potentialscf.cf_875 = 'Active'",
							[$mobile]
						);				
						if ($adb->num_rows($result) > 0) {
							$response = new Mobile_API_Response();
							$response->setError("TRIAL_EXISTS", "A trial is already associated with this mobile number.");
							return $response;
						}
					}
				}
