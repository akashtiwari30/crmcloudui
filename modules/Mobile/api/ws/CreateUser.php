...
class Mobile_WS_CreateUser extends Mobile_WS_FetchRecordWithGrouping
{
    ...
    function process(Mobile_API_Request $request)
    {
        ...
        if (!empty($values)) {
            ...
            if (empty($values['phone'])) {
                ...
                $response->setError(1501, "Phone Number is Invalid");
                return $response;
            }
            if (empty($values['email'])) {
                http_response_code(400);
                $response->setError(1501, "Email is Invalid");
                return $response;
            }
        }
            ...
            if (empty($recordid) || vtws_recordExists($recordid)) {
                ...
                if ($module == 'Leads') {
                    ...
                    if (!empty($mobile)) {
                        global $adb;
                        $result = $adb->pquery(
                            "SELECT vtiger_crmentity.crmid, vtiger_potentialscf.cf_853 
                            ...
                            WHERE vtiger_potentialscf.cf_861 = ? 
                            AND vtiger_crmentity.deleted = 0 
                            AND vtiger_potentialscf.cf_875 = 'Active'",
                            [$mobile]
                        );
                        ...
                        if ($adb->num_rows($result) > 0) {
                            ...
                            $response = new Mobile_API_Response();
                            http_response_code(400);
                            $response->setError("DUPUSR", "User already has one CRM instance, please use this CRM: " . $row['cf_853']);
                            return $response;
                        }
                    }
                }
