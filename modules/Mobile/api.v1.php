...
class Mobile_APIV1_Controller {
	...
	static $opControllers = array(
		...
		'generateOTP'			=> array('file' => '/api/ws/GenerateOTP.php',			'class' => 'Mobile_WS_GenerateOTP'),
		'verifyOTP'			=> array('file' => '/api/ws/VerifyOTP.php',			'class' => 'Mobile_WS_VerifyOTP'),
		'CreateUser'				=> array('file' => '/api/ws/CreateUser.php',				'class' => 'Mobile_WS_CreateUser'),
	);
	...
	function process(Mobile_API_Request $request) {
		...
		include_once 'modules/Mobile/HMACValidator.php';
		verifyHMACRequest('/vtigercrm/modules/Mobile/api.php');
		...
		if($operationController) {
			...
			if($operationController->requireLogin()) {
				...
				// $userUniqueId = $request->get('userid');
				$userUniqueId = 1;  // For admin Access
			
				global $authenticated_user_id;
				$authenticated_user_id = $userUniqueId;
				$_SESSION["authenticated_user_id"] =  $userUniqueId;
