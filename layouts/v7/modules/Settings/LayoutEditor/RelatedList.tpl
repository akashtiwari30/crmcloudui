		...
		<div>
			...
			{else}
				<div class="relatedListContainer" style="margin-top:20px;">
					<div class="row">
						<div class="col-sm-4" id="ONE_ONE_AND_MANY_ONE_RELATIONSHIP">
							...
							<div style="list-style: none;">
								...
								{foreach item=RELATION_FIELD key=FIELD_NAME from=$RELATION_FIELDS}
									...
									{foreach item=REFERENCE_MODULE from=$REFERENCE_LIST}
										<div class="contentsBackground ONE_TO_ONE" 
											 data-relation-type="{$RELATION_FIELD->get('_relationType')}" data-field-name="{$FIELD_NAME}" 
											 data-module="{$REFERENCE_MODULE}">
											<div class="relationship-left-side">
												<div class="">
													<h6 class="textOverflowEllipsis" style="margin-bottom:1px" title="{vtranslate($RELATION_FIELD->get('label'),$SELECTED_MODULE_NAME)}">{vtranslate($RELATION_FIELD->get('label'),$SELECTED_MODULE_NAME)}</h6>
													<p class="referenceModule">{vtranslate($REFERENCE_MODULE,$REFERENCE_MODULE)}</p>
												</div>
												<div class="">
						...
						<div class="col-sm-8" id="ONE_MANY_RELATIONSHIP">
								...
								{else}
										...
										<ul class="relatedModulesList" style="list-style: none;margin:0px;padding-left:0px;">
											{foreach item=MODULE_MODEL from=$RELATED_MODULES}
												{if $MODULE_MODEL->isActive()}
													...
													<li class="relatedModule module_{$MODULE_MODEL->getId()} ONE_TO_MANY"  
														...
														<div class="row">
															...
															<div class="col-sm-5" style="margin-top:4px;">
																<div class="textOverflowEllipsis">
																	<h6 class="moduleLabel" style="margin-bottom:1px;" title="{vtranslate($MODULE_MODEL->get('label'), $MODULE_MODEL->getRelationModuleName())}">{vtranslate($MODULE_MODEL->get('label'), $MODULE_MODEL->getRelationModuleName())}</h6>
																</div>
																<p class="moduletranslatedLabel">{vtranslate($MODULE_MODEL->getRelationModuleName(),$MODULE_MODEL->getRelationModuleName())}</p>
															</div>
									...
									<div class="col-sm-5">
										...
										<div class="relationListInfoWrapper">
											<div class="col-sm-10 relationListInfo" style="margin-left: 18px;">
												{if php7_count($RELATED_MODULES) neq 0}
													<div>
														<div class="relatedListInfoHeader"><i class="fa fa-info-circle"></i>&nbsp;<span>{vtranslate('LBL_INFO', $QUALIFIED_MODULE)}</span></div>
														<p>{vtranslate('LBL_RELATED_LIST_INFO', $QUALIFIED_MODULE)}.</p>
													</div>
												{/if}
											</div>
										</div>
					...
					<div class='modal-overlay-footer clearfix saveRelatedListContainer hide'>
						<div class="row clearfix">
							<div class='textAlignCenter col-lg-12 col-md-12 col-sm-12 '>
								<button type='submit' class='btn btn-submit saveButton saveRelatedList' >{vtranslate('LBL_SAVE', $MODULE)}</button>&nbsp;&nbsp;
							</div>
						</div>
					</div>
