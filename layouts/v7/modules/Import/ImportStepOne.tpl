...
<div class ="importBlockContainer show" id = "uploadFileContainer">
    <table class = "table table-borderless bg-light-color-2" cellpadding = "30" >
        <div>
			{if $FORMAT eq 'vcf'}
				<h5>{'LBL_IMPORT_FROM_VCF_FILE'|@vtranslate:$MODULE}</h5>
			{else if $FORMAT eq 'ics'}
				<h5>{'LBL_IMPORT_FROM_ICS_FILE'|@vtranslate:$MODULE}</h5>
			{else}
				<h5>{'LBL_IMPORT_FROM_CSV_FILE'|@vtranslate:$MODULE}</h5>
			{/if}
        </div>
        <tr id="file_type_container">
			{if $FORMAT eq 'vcf'}
				<td><p>{'LBL_SELECT_VCF_FILE'|@vtranslate:$MODULE}</p></td>
			{else if $FORMAT eq 'ics'}
				<td><p>{'LBL_SELECT_ICS_FILE'|@vtranslate:$MODULE}</p></td>
			{else}
				<td><p>{'LBL_SELECT_CSV_FILE'|@vtranslate:$MODULE}</p></td>
			{/if}
        ...
        {if $FORMAT eq 'csv'}
            <tr id="has_header_container">
                <td><p>{'LBL_HAS_HEADER'|@vtranslate:$MODULE}</p></td>
                <td>
        ...
        {if $FORMAT eq 'csv'}
            <tr id="delimiter_container">
                <td><p>{'LBL_DELIMITER'|@vtranslate:$MODULE}</p></td>
                <td>
